...
n,m=map(int,sys.stdin.readline().split()); bad=set(sys.stdin.readline().split()); people=items=0
for _ in range(n):
    a=sys.stdin.readline().split(); found=[x for x in a[2:] if x in bad]
    if found: print(a[0]+':',' '.join(found)); people+=1;items+=len(found)
print(people,items)
