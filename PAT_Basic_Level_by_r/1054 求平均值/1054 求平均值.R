...
# 空间复杂度：O(n)
x<-readLines("stdin",warn=FALSE);a<-strsplit(x[2],"\\s+")[[1]];ok<-vapply(a,function(s)grepl("^[+-]?(\\d|[1-9]\\d)(\\.\\d{1,2})?$",s)&&abs(as.numeric(s))<=1000,logical(1));for(s in a[!ok])cat("ERROR: ",s," is not a legal number\n",sep="");v<-as.numeric(a[ok]);if(!length(v))cat("The average of 0 numbers is Undefined\n") else cat(sprintf("The average of %d number%s is %.2f\n",length(v),ifelse(length(v)==1,"","s"),mean(v)))
