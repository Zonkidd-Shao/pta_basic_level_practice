...
//
// 实现原理：
// 本题要求计算两个复数的乘积。复数以极坐标形式给出：每个复数由半径 r 和辐角 p（角度制）表示。
...
//
// 关键步骤：
// 1. 读取两个复数的极坐标参数 (r1, p1) 和 (r2, p2)
// 2. 计算乘积的模长 r = r1 * r2
// 3. 将角度从度转换为弧度：ang = (p1 + p2) * π / 180
...
int main() {
    ...
    // 计算乘积的模长：极坐标下模长相乘
    double r = r1 * r2;
    // 角度制转弧度制：π = acos(-1)，弧长公式 ang = (p1+p2) * π / 180
    double ang = (p1 + p2) * acos(-1.0) / 180.0;
